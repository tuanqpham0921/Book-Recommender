...
| counting / previewing / fetching / streaming books | `BookWorkflow` — `count_books`, `preview_books`, `fetch_anchor_books`, `stream_books`, `self.store` |
...
- **The book node set.** The V1 taxonomy is `Retrieve_by_ISBN13`, `Retrieve_by_Title`, `Retrieve_by_Author`, `Retrieve_by_CoAuthors`, `Retrieve_by_Category`, `Retrieve_by_Numeric_Traits`, `Retrieve_Random`, `Combine_Union`, `Combine_Intersect`, `Filter_Retrieval`, `Analyze_Compare`, `Analyze_Recommend`. **On `minimal_end_to_end_v1` only the four retrieval nodes `Retrieve_by_Title`, `Retrieve_by_Author`, `Retrieve_by_Category` and `Retrieve_by_Numeric_Traits` are registered** — the rest are parked while the end-to-end path is built, so the rules below describe the target taxonomy, not what the planner can currently emit. `Filter_Retrieval` and `Analyze_Recommend` joined them on 2026-08-22, which leaves the catalog **retrieval-only**: `NodeTier.COMBINE` and `NodeTier.ANALYZE` have no members, so those headings do not render and no plan can have a second stage. Two things follow that the target rules below still assume: a bound riding alongside a subject has nowhere to go, and a plan is now always a set of independent retrievals pooled by definition. `make tools-catalog` prints what is actually live. Each retrieval node is single-dimension (no cross-column filtering) and single-valued — one title, one author per node; several authors' separate bibliographies means several `Retrieve_by_Author` nodes. **Two nodes are documented exceptions on *dimension*, both because their subject has no single dimension to be single about.** `Retrieve_by_Numeric_Traits` carries a whole `BookMetadataFilter`, so bounds can be the **subject** of a search ("books under 200 pages", "well rated books") rather than only a narrowing of one; it fires only when the numbers are the entire request — any other subject present and the bounds belong to `Filter_Retrieval` or to `Analyze_Recommend`'s goal text. That numbers-only rule is prose, not schema, so eval cases 76/77 are what hold it. `Retrieve_by_Category` carries subject keywords + fiction-ness + audience, ANDed, because "non-fiction about history" is one question rather than two to intersect; it is the node for **anything that is not a number and not an identifier**. Its line against `Analyze_Recommend` is mechanism, not subject matter: `Retrieve_by_Category` is **lexical** (does the catalog's text contain these words — a tsquery over title + shelf + blurb, handing on a composable query over the whole match), `Analyze_Recommend` is **semantic** (which books are near an embedding — a ranked terminal choice). "Cozy mysteries" splits: *mystery* is a word the text contains, *cozy* is a feel no word search can find. Note `genre` there means only fiction/non-fiction, the two values `books.genre` distinguishes; every finer shelf word ("mystery", "history") is a keyword. `Retrieve_by_CoAuthors` is the one multi-value node and means joint works only (2+ authors ANDed onto the same book), not a combined bibliography — see [docs/design/node-taxonomy-v1.md](docs/design/node-taxonomy-v1.md). `Retrieve_Random` is the one node that carries a full `BooksFilter` (a random pick has no dimension to be single about); it serves the bare "recommend me a book" **alone**, with no `Analyze_Recommend` after it, and `Filter_Retrieval` may never depend on it — bounds on a surprise go in its own `filters`. Cross-dimension requests are expressed by composition instead: the **combine tier** (`NodeTier.COMBINE`: `Combine_Union` = OR, `Combine_Intersect` = AND, `Filter_Retrieval` = metadata narrowing) consumes prior retrieval output and searches for nothing of its own — it can only shrink or merge what earlier goals found. Under counts-first that is still SQL: `Filter_Retrieval` ANDs its bounds onto the upstream *query* (`BookStore.filter_query`) and counts the narrowed set, rather than deleting from rows someone fetched. Pooling is also implicit — several task ids in one `depends_on` are pooled (OR) by definition — so `Combine_Union` is only used when that pooled set is an explicit step something downstream consumes — see [docs/design/execution-pipeline-v1.md](docs/design/execution-pipeline-v1.md). This domain's specs are listed in `app/domains/books/guide.py`; `app/registry.py` composes it with the other domains.
