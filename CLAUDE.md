...
- **`OperationResult[T]`** — the result envelope, and there is exactly **one** of them: `ok`, `input`, `output`, `details`, `runtime_error`, `duration`, `token_usage`, `id`, `parent_id`, `steps`, plus `add_step`/`unwrap`/`flatten`/`to_span`. A `Workflow` and a `@task` produce the same class; one that ran nothing else just carries an empty `steps`. The old `WorkFlowOperationResult` subclass that added `steps` is **gone** — `steps` was never mandatory (an empty list costs nothing, `flatten` reads it the same either way), and once nesting became automatic any unit of work can run another, so "which shape am I" stopped being answerable at decoration time. What the split produced in practice was the same relationship rebuilt from several angles: an isinstance ladder in `flatten`, a `getattr(x, "steps", [])` at every reader, and a rule about who was allowed to call whom. Parent callers access child output via `.output`. There is deliberately **no `message` field** — it was write-only noise that every layer overwrote; free-text goes in `details` (via `add_details`), and failure text lives on `runtime_error.message`. **`add_step()` lives on the envelope, not on `Workflow`** — it appends a child, stamps its `parent_id`, and rolls its `token_usage` up, so a non-`Workflow` caller (the `Orchestrator`) can build a root envelope over workflows that each own their own record. **`parent_id` is set there and nowhere else**, and stamping it at attach time rather than deriving it later is what makes the link survive the JSONB insert. **`add_step` is idempotent** — a step whose `parent_id` is already set is skipped (and one already claimed by a *different* parent is refused and logged), so the automatic and explicit attach paths below can both be live without appending a step twice or counting its tokens twice.

- **Nesting is automatic — one ContextVar** (`airglider/src/context.py`). A child cannot know its own parent, so instead the *caller* publishes its own envelope for the duration of the call: `parent_scope(record)` sets `CURRENT_PARENT`, and in its `finally` resets it and then attaches `record` to whatever was current before. **`record_span` (`airglider/src/span.py`) is the only call site**, which is what keeps the set/reset discipline checkable by reading one function. It is the recording body `@task`'s wrapper and `Workflow.__call__` share — timing pair, `parent_scope`, a cancel path that stamps and re-raises, a **stop path for `StepFailure`**, an error path that stamps and does not re-raise, and a `finally` closing `duration` inside the scope — parametrized only by the noun in its log lines. Each caller keeps what is genuinely its own: the workflow `check_output_type` and the single-use guard; the task its return-value wrapping. **The `StepFailure` branch is here rather than in `Workflow.__call__`** so a `@task` that aborts records and logs identically to a workflow that does — which decorator a unit of work happens to use is not a fact about the failure. It stamps `runtime_error` like any other failure (a stop is not a completion, so it must carry a reason) but logs one warning line naming the step, with no traceback: the step that actually raised already logged the real one, and reprinting it at every level above is how one cause becomes four tracebacks. `record_call_input` in `airglider/src/utils.py` is likewise the one implementation of "bind the arguments, summarize them, never raise", used by both. Consequences worth knowing: **who may call whom no longer matters** — a task can call a task, a workflow, or a mix, and the tree comes out right without anything being threaded through the signature; `run_async_step` is no longer what attaches a step (its `add_step` is a no-op that the idempotency guard absorbs), and its remaining job — the failure policy — is **superseded by two verbs that read better and work inside a `@task` too**: `await step` hands back the envelope and leaves the caller to decide, `(await step).unwrap()` hands back the payload or raises `StepFailure`. That splits awaiting from the policy, so a caller can retry an envelope, or inspect it and *then* insist. `unwrap` notes `FAILED STEP: <name>` on whatever envelope is currently being built (via `current_parent()`) before raising; when the failed step carries **no** `runtime_error` the message says so, because under "`ok` = ran to completion" that combination is a bug in the step rather than a state to report, and without it the caller would stop with nothing underneath explaining why. Attaching on the way *out* rather than in is required by the token rollup (`add_step` reads usage once, at attach time), and gets the cancel path for free: `finally` runs while `CancelledError` propagates, so a workflow killed by a client disconnect still lands in its parent's `steps`. A plain `await` shares the caller's context; `gather`/`create_task` copy it, so concurrent siblings each keep their own parent — the copy is shallow, so the attach still mutates the real record. **Fire-and-forget is the case that stays broken**: a `create_task` outliving its parent attaches to an envelope already serialized and reported. Await background work inside the scope that owns it.

- **A `@task` returns its payload; the envelope is the decorator's.** Returning an `OperationResult` **raises `TypeError`**, which `record_span` records as that task's own failure with a message naming the replacements. The shape used to mean two things — "report `ok` myself without raising" and "hand back what I called" — and both have better answers now: `ok` means ran-to-completion so nobody votes on it, and anything awaited inside the body already attached itself via `parent_scope`. What was left were only the ways it goes wrong, so it is rejected rather than silently unwrapped: unwrapping would read a *failed* step as a success with a `None` payload, and storing the envelope in `response` would serialize the subtree twice and make `.result` hand back a record. The replacements: `(await step).unwrap()` to pass a nested step's payload through, `await step` to inspect it, and **`airglider.add_details(...)`** to attach free text to the envelope currently running — which inside a `@task` body is that task's own record, since `record_span` published it before the call. (Note `Workflow.add_details` always writes to *that workflow's* record, so in a `@task` method on a workflow the two land in different places.)

- **`flatten()`** returns the tree as a depth-first span list, parent before child; with `parent_id` and `timing.end_time` on every entry the nesting is rebuildable from the list alone, which is the shape a timeline or per-step cost table wants. No row drags a subtree — that would serialize the tree once per level — so every node goes through **`to_span()`** on the way in, which returns a shallow copy carrying `steps=[]` (`model_construct`, so the payload is not dumped to a dict); a node with no children has nothing to drop and comes back **by reference**. **`steps` is typed `list[Any]` on purpose**: pydantic would otherwise re-validate a child on assignment and hand back a *copy*, breaking the one thing the tree depends on — a step being the same object the workflow that produced it is still writing to. The cost is plain-dict children after a JSON round trip, which `flatten` validates on the way past. **`to_summary()`** renders the tree recursively as one small dict per envelope (id/short name/ok/duration/tokens/error plus the payload's own `to_summary`, empties dropped, `steps` nested beneath) — a readable companion to the full tree, never a replacement for it.

- **`Workflow`** — for multi-step async processes. Subclass and override `run()`. Centralizes start/error logging and catches runtime exceptions without crashing. Each `Workflow` owns one `OperationResult` in memory; steps append to `.steps` as they complete, by adopting themselves out of `parent_scope` (see the ContextVar bullet above). **`__call__` stamps `record.input` before running `run()`** — see the `@task` bullet below; both record the same way. `UserFacingBaseWorkflow` adds SSE streaming helpers.
- **`@task` decorator** — for a single async function. Wraps it, catches exceptions, and returns an `OperationResult` — the same class a `Workflow` owns, so a task that calls other work holds it as `steps`. To attach `details` from inside a `@task`, call `airglider.add_details(...)`; returning a hand-built `OperationResult` is rejected, as described above. **`Workflow` vs `@task` is now about how much scaffolding you want, not about what you are allowed to call**: a `Workflow` gives you an output type, SSE helpers and a class to hang state on; a `@task` is a decorated function. Either can call either, and since `unwrap` lives on the envelope, either can abort on a failed step the same way.
