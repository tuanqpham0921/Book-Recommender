...
- **`OperationResult[T]` / `OperationResult[T]`** — the result envelope, split in two by shape. `OperationResult` is one unit of work: `ok`, `input`, `output`, `details`, `runtime_error`, `duration`, `token_usage`, `id`, `parent_id`. `OperationResult` subclasses it and adds `steps` — and `add_step`/`flatten`/`to_span`, the three things that only make sense over children. **Type on the base class unless the code actually walks the tree**: `run_async_step`, the orchestrator's isinstance guard and every annotation that only reads `ok`/`result` take the narrower type. Note that **`@task` now builds a `OperationResult` too** — a task that runs another unit of work has to be able to hold it, and a leaf-shaped record could publish itself as the current parent but never adopt anything (grandchildren would silently skip a level). It is still a subclass, so every `OperationResult` annotation keeps holding; a task that runs nothing else just carries an empty `steps`. Parent callers access child output via `.output`. There is deliberately **no `message` field** — it was write-only noise that every layer overwrote; free-text goes in `details` (via `add_details`), and failure text lives on `runtime_error.message`. **`add_step()` lives on the envelope, not on `Workflow`** — it appends a child, stamps its `parent_id`, and rolls its `token_usage` up, so a non-`Workflow` caller (the `Orchestrator`) can build a root envelope over workflows that each own their own record. **`parent_id` is set there and nowhere else**, and stamping it at attach time rather than deriving it later is what makes the link survive the JSONB insert. **`add_step` is idempotent** — a step whose `parent_id` is already set is skipped (and one already claimed by a *different* parent is refused and logged), so the automatic and explicit attach paths below can both be live without appending a step twice or counting its tokens twice.
