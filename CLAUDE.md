...
- **The book node set.** The V1 taxonomy is `Retrieve_by_ISBN13`, `Retrieve_by_Title`, `Retrieve_by_Author`, `Retrieve_by_CoAuthors`, `Retrieve_by_Lexical_Traits` (renamed from `Retrieve_by_Category` 2026-08-24), `Retrieve_by_Numeric_Traits`, `Retrieve_Random`, `Combine_Union`, `Combine_Intersect`, `Filter_Retrieval`, `Analyze_Compare`, `Analyze_Similar_Books` (renamed from `Analyze_Recommend` 2026-08-22). **On `minimal_end_to_end_v1` only five are registered** — the four retrieval nodes `Retrieve_by_Title`, `Retrieve_by_Author`, `Retrieve_by_Lexical_Traits`, `Retrieve_by_Numeric_Traits`, plus `Analyze_Similar_Books` — so the rules below describe the target taxonomy, not what the planner can currently emit. `Filter_Retrieval` stays parked, which means `NodeTier.COMBINE` has no members and that heading does not render; a bound riding alongside a subject therefore has nowhere to go, and the only two-stage plan available is `Retrieve_by_Title` → `Analyze_Similar_Books`. `make tools-catalog` prints what is actually live. Each retrieval node is single-dimension (no cross-column filtering) and single-valued — one title, one author per node; several authors' separate bibliographies means several `Retrieve_by_Author` nodes. **Two nodes are documented exceptions on *dimension*, both because their subject has no single dimension to be single about.** `Retrieve_by_Numeric_Traits` carries a whole `BookMetadataFilter`, so bounds can be the **subject** of a search ("books under 200 pages", "well rated books") rather than only a narrowing of one; it fires only when the numbers are the entire request — any other subject present and the bounds belong to `Filter_Retrieval`. (They used to have a second home in `Analyze_Recommend`'s goal text; `Analyze_Similar_Books` parses nothing, so that home is gone — see the taxonomy doc.) That numbers-only rule is prose, not schema, so eval cases 76/77 are what hold it. `Retrieve_by_Lexical_Traits` carries subject keywords + fiction-ness + audience, ANDed, because "non-fiction about history" is one question rather than two to intersect; it is the node for **anything that is not a number and not an identifier**. Its line against `Analyze_Similar_Books` is mechanism, not subject matter: `Retrieve_by_Lexical_Traits` is **lexical** (does the catalog's text contain these words — a tsquery over title + shelf + blurb, handing on a composable query over the whole match), `Analyze_Similar_Books` is **semantic** (which books are near an embedding — a ranked pool for a later step to pick from). They no longer split a single ask between them, though: the similarity node takes only named books, so "cozy mysteries" is `Retrieve_by_Lexical_Traits` alone with the feel dropped, not lexical + semantics. Note `genre` there means only fiction/non-fiction, the two values `books.genre` distinguishes; every finer shelf word ("mystery", "history") is a keyword. `Retrieve_by_CoAuthors` is the one multi-value node and means joint works only (2+ authors ANDed onto the same book), not a combined bibliography — see [docs/design/node-taxonomy-v1.md](docs/design/node-taxonomy-v1.md). `Retrieve_Random` is the one node that carries a full `BooksFilter` (a random pick has no dimension to be single about); it serves the bare "recommend me a book" **alone**, with no analyze node after it, and `Filter_Retrieval` may never depend on it — bounds on a surprise go in its own `filters`. Cross-dimension requests are expressed by composition instead: the **combine tier** (`NodeTier.COMBINE`: `Combine_Union` = OR, `Combine_Intersect` = AND, `Filter_Retrieval` = metadata narrowing) consumes prior retrieval output and searches for nothing of its own — it can only shrink or merge what earlier goals found. Under counts-first that is still SQL: `Filter_Retrieval` ANDs its bounds onto the upstream *query* (`BookStore.filter_query`) and counts the narrowed set, rather than deleting from rows someone fetched. Pooling is also implicit — several task ids in one `depends_on` are pooled (OR) by definition — so `Combine_Union` is only used when that pooled set is an explicit step something downstream consumes — see [docs/design/execution-pipeline-v1.md](docs/design/execution-pipeline-v1.md). This domain's specs are listed in `app/domains/books/guide.py`; `app/registry.py` composes it with the other domains.
...
- Repository pattern in `db/stores/` — `book_store.py` is the primary store. Retrieval is **counts-first**: `title_query()` builds a `DeferredBookQuery` that isn't run for rows, `filter_query()` ANDs metadata bounds onto one and hands back another, `count()` runs only a `COUNT`, `DeferredBookQuery.compose()` folds several into one CTE, and `materialize()` is the single place rows are fetched — at the end of a plan. Statement derivation lives on `DeferredBookQuery` itself; the store builds from dimensions and executes — with one exception, `embedding_search_stmt`, a module-level pure builder rather than a store method, because only the caller knows the label that elides its 1024-float vector from the recorded SQL; keeping that build step out of the store is what keeps airglider out of `db/`. **`DeferredBookQuery` carries no LIMIT and no ORDER BY — with one documented exception** (2026-08-24): the vector search keeps both, because it does not select a subset but orders the whole table and truncates, so the cap *is* the pool. Because its `score` is cosine similarity, `filter_query()` narrows it and `materialize_stmt()` re-orders by that score — so **a bound on a similarity ask does not flatten the ranking**, which is what makes it two nodes rather than a parse inside the similarity node. `score_stats()` is its counting call, since `count()` on it only ever reports the LIMIT. **Nothing on the class marks the exception and `compose()` does not refuse it** — a `capped` attribute and a compose guard were built and removed the same day, because no registered plan composes a pool (empty combine tier, parked single-input `Filter_Retrieval`). Composing one *is* lossy — the LIMIT applies before the union, `score` is dropped after — so tune `MIN_SIMILARITY` to bound the pool or restore the guard before unparking the combine tier. See `db/README.md` and [docs/design/execution-pipeline-v1.md](docs/design/execution-pipeline-v1.md)
