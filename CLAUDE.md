...
- **`@task` decorator** — for a single async function. Wraps it, catches exceptions, and returns an `OperationResult` — the same class a `Workflow` owns, so a task that calls other work holds it as `steps`. To set `ok` yourself or attach `details`/`output` from inside a `@task`, return a custom `OperationResult` directly — the decorator merges it, as described above. **`Workflow` vs `@task` is now about how much scaffolding you want, not about what you are allowed to call**: a `Workflow` gives you an output type, SSE helpers and a class to hang state on; a `@task` is a decorated function. Either can call either, and since `unwrap` lives on the envelope, either can abort on a failed step the same way.
