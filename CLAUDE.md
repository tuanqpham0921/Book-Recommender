...
- **Shared output shapes** — `app/domains/books/schemas.py` (`BookSummary`, `BookRetrievalOutput`, `BookRecommendationOutput`). A node's own output subclasses the shape its docstring claims in `Returns:`.
