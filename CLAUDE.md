...
**Current state:** `TaskRunnerWorkflow` (`app/orchestration/task_runner.py`) executes the classified strategies against the real executors, for the node types registered on this branch. It brackets each node with `task.start`/`task.end` SSE events so the UI can render one collapsible section per step (count in the header, preview cards inside); see `frontend/src/README.md`. **Then it writes the turn's prose**: after the layers drain it attaches one `AnswerWorkflow` (`app/domains/books/write_answer/`) to every **sink** of the plan — `PlanJaneOutput.branches()`, a sink plus its ancestors, sink last — materializes each branch's queries, streams the cards, and streams a reply. The stage is **not in the registry**: no `SPEC`, no `NodeTypeEnum` member, no catalog entry, so the planner can neither choose it nor omit it and no eval golden carries it (see `docs/design/execution-pipeline-v1.md`). One reply per sink, not per turn — a chained ask ("do you have Dune? and recommend like it") is one branch and one reply; a compound ask ("Dune by Jane Austen? and recommend like IT") is two of each. A branch is answered **whether or not its sink succeeded**, which is how "I don't have Dune, so I couldn't find anything like it" gets written: only the ancestor knows why the sink failed. Conversation is **single-turn**: each request is processed statelessly (turns are recorded to `chat_runs` but never read back).
...
- **Naming (`Workflow` / `Executor`)** — the ladder is `airglider.Workflow` → `AppWorkflow` (`app/domains/base_workflow.py`) → `BookReaderWorkflow` → `BookWorkflow` (both `app/domains/books/base_workflow.py`). Concrete units of work are `*Workflow` too (`TriageWorkflow`, `TaskRunnerWorkflow`, `AnswerWorkflow`). **`Executor` is the subset of those the planner can dispatch** — a node with a request schema, a `NodeSpec` and a catalog entry, reached via `REGISTRY.spec(...).executor` instead of called directly. Node vs. pipeline step is what the second word carries. (The old "`Base` marks a reusable base class" rule is retired — see `backend/app/domains/README.md`.) **The last two rungs split on whether the output is book-shaped**: `BookReaderWorkflow` holds `store`/`fetch_books`/`stream_books`, none of which writes an output field, so it is bound to `NodeWorkflowOutput`; `BookWorkflow` adds `count_books`, which writes `query`/`query_sql`/`num_books`, so it is bound to `BookRetrievalOutput`. Every node subclasses `BookWorkflow`; `AnswerWorkflow` is the one subclass of the reader alone, because it needs rows and cards while producing prose. `BookWorkflow` carries `count_books(query)` and `fetch_books(query, limit)` — both `@task`s: the first stamps `query`/`query_sql`/`num_books` on the output and fetches nothing, the second hands rows back for the caller to use as it likes (a preview to stream and drop, or a capped anchor to keep) — plus `stream_books()`, which validates through `BookOut` before anything reaches the browser. There is deliberately no third: a node needing more composes it in its own flow, which is what replaced `fetch_anchor_books()` and its single caller. The counting/fetching pair replaced a single `preflight()` that returned `(total, sample)` from one round trip; splitting them is what let `BookRetrievalOutput.books` go away.
