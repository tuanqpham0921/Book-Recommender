...
export const BookCard = ({ book }) => {
  return (
    <div className="bg-[var(--bg-primary)] rounded-2xl shadow-sm flex flex-col h-full p-3">
      <div className="flex flex-col h-full">
        ...
        <h3 className="font-bold text-base sm:text-lg md:text-xl text-[var(--text-active)] mb-2 line-clamp-2">
          {book.title}
        </h3>

        <div className="mt-auto">
          <p className="text-[var(--text-hover)] text-sm md:text-md line-clamp-2">
            <span className="hidden sm:inline">{formatAuthors(book, 2)}</span>
            <span className="sm:hidden">{formatAuthorsMobile(book)}</span>
          </p>
          <p className="text-[var(--text-hover)] text-xs md:text-sm line-clamp-1">
...
export const BookCardDetailed = ({ book }) => {
  ...
  return (
    <div className="bg-[var(--bg-primary)] border border-[var(--border-light)] rounded-lg p-4 mb-2 flex-shrink-0">
      <div className="flex gap-4">
        ...
        <div className="flex-1 min-w-0">
          <h3 className="font-semibold text-base sm:text-lg text-[var(--text-active)] mb-1 line-clamp-2">
            {book.title} <span className="text-[var(--text-muted)] text-xs sm:text-sm ml-1"> - {book.published_year}</span>
          </h3>

          <div className="flex flex-col gap-1 text-sm text-[var(--text-hover)] mb-2">
            <p className="m-0">
      ...
      {book.description && (
        <div className="border-t border-[var(--border-light)] pt-3">
          <div className="text-sm text-[var(--text-hover)]">
            ...
            {book.description.length > DESCRIPTION_CHAR_LIMIT && (
              <button
                ...
                className="text-[var(--text-inactive)] hover:text-[var(--text-active)] font-medium mt-2 text-sm"
              >
