...
def test_execution_order_single_retrieval(strategy_wf):
...
def test_execution_order_simple_chain(strategy_wf):
    r = _make_retrieval("task_1")
...
def test_execution_order_parallel_retrievals(strategy_wf):
...
def test_execution_order_fan_in(strategy_wf):
    r1 = _make_retrieval("task_1", title="Book A")
    r2 = _make_retrieval("task_2", title="Book B")
...
def test_cycle_produces_empty_execution_order(strategy_wf):
...
def test_cycle_moves_nodes_to_refused(strategy_wf):
    a = _make_analyze("task_1", depends_on_ids=["task_2"])
    b = _make_analyze("task_2", depends_on_ids=["task_1"])
    strategy_wf.output.accepted = [a, b]
    strategy_wf.create_execution_order()
...
def test_good_strategy_goes_to_accepted(strategy_wf):
    goal = _make_goal()
...
def test_low_confidence_strategy_goes_to_refused(strategy_wf):
    goal = _make_goal()
    r = _make_retrieval(id_str="task_1", goal_id=goal.id)
...
def test_missing_target_goal_goes_to_refused(strategy_wf):
    goal = _make_goal()
...
def test_set_llm_id_replaces_id_with_internal_format(strategy_wf):
    r = _make_retrieval("task_1")
...
def test_set_llm_id_returns_complete_mapping(strategy_wf):
...
def test_map_dependencies_translates_llm_ids(strategy_wf):
    r = _make_retrieval("task_1")
    a = _make_analyze("task_2", depends_on_ids=["task_1"])
...
def test_map_dependencies_flags_refusal_on_missing_dep(strategy_wf):
...
def test_inject_retrieval_when_only_analyze_present(strategy_wf):
...
def test_no_injection_when_retrieval_already_present(strategy_wf):
...
def test_no_injection_when_only_retrieval_present(strategy_wf):
