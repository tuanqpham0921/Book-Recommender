"""Tests for `BookStore.category_query` and the two pure helpers behind it.
...
def _category(**kwargs) -> DeferredBookQuery:
    # the session is never touched: category_query only builds
    return BookStore(MagicMock()).category_query(**kwargs)
...
class TestIndexMatchGuard:
    ...
    def test_regconfig_is_inlined_not_bound(self):
        compiled = _compiled_sql(_category(keywords=["ninja"]).stmt)
    ...
    def test_keyword_is_bound_not_spliced(self):
        compiled = _compiled_sql(_category(keywords=[INJECTION_PAYLOAD]).stmt)
...
class TestCategoryQuery:
    def test_refuses_empty_args(self):
        with pytest.raises(ValueError, match="empty category filter"):
            _category()
    ...
    def test_keywords_become_one_tsquery(self):
        ...
        compiled = _compiled_sql(_category(keywords=["cozy", "mystery"]).stmt)
    ...
    def test_keyword_query_carries_a_score(self):
        assert "AS score" in _compiled_sql(_category(keywords=["ninja"]).stmt)
    ...
    def test_shelf_only_query_carries_no_score(self):
        ...
        compiled = _compiled_sql(_category(audience=AudienceEnum.CHILDREN).stmt)
    ...
    def test_genre_is_set_membership_never_like(self):
        ...
        compiled = _compiled_sql(_category(genre=GenreEnum.FICTION).stmt, True)
        assert "books.genre IN" in compiled
        assert "ILIKE" not in compiled.upper()

    def test_facets_are_anded(self):
        compiled = _compiled_sql(
            _category(keywords=["space"], genre=GenreEnum.NONFICTION).stmt
        )
        assert "@@" in compiled
        assert "books.genre IN" in compiled
    ...
    def test_keeps_the_deferred_invariants(self, kwargs):
        ...
        compiled = _compiled_sql(_category(**kwargs).stmt).upper()
        assert "LIMIT" not in compiled
        assert "ORDER BY" not in compiled

    def test_selects_isbn13(self):
        assert "books.isbn13" in _compiled_sql(_category(keywords=["ninja"]).stmt)

    def test_is_labelled_category(self):
        assert _category(keywords=["ninja"]).label == "category"
