...
Every class here is a candidate capability for the planner to select; none has
an executor yet. They exist to evaluate how parse_intent and
strategy_classification scale as the tool catalog grows (see
tests/query_suite_extended.json). Docstrings are the tool descriptions the
LLM sees, so they follow the same "use when / not when" style as
request_schemas.py — keep them discriminating against neighbor tools.
"""
